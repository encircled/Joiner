package cz.encircled.joiner.springbootexample.controller

import cz.encircled.joiner.kotlin.JoinerKt
import cz.encircled.joiner.kotlin.JoinerKtOps.eq
import cz.encircled.joiner.kotlin.JoinerKtQueryBuilder.all
import cz.encircled.joiner.kotlin.JoinerKtQueryBuilder.from
import cz.encircled.joiner.springbootexample.CustomerDto
import cz.encircled.joiner.springbootexample.QPerson.person
import org.springframework.beans.factory.annotation.Autowired
import org.springframework.web.bind.annotation.GetMapping
import org.springframework.web.bind.annotation.PathVariable
import org.springframework.web.bind.annotation.RequestMapping
import org.springframework.web.bind.annotation.RestController


@RestController
@RequestMapping("/person")
class PersonController {

    @Autowired
    lateinit var joiner: JoinerKt

    @GetMapping("{id}")
    fun getOne(@PathVariable id: Long): CustomerDto {
        return joiner.getOne(person.all() where { it.id eq id }).toDto()
    }

    @GetMapping
    fun getAll(): List<CustomerDto> {
        return joiner.find(person.all()).map { it.toDto() }
    }

    @GetMapping("/names")
    fun getNames(): List<String> {
        return joiner.find(person.name from person)
    }

}